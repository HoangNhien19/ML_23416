...
class UIPrediction:
    ...
    def create_ui(self):
        ...
        self.view_dataset_button = Button(choose_dataset_panel, text="2. View Dataset",
                                          width=20, command=self.do_view_dataset)
        ...
        percent_label = Label(text="%", width=2, anchor="w", justify=LEFT)
        ...
        self.train_model_result_label = Label(training_rate_panel, text=self.status.get(),
        ...
        # --- Load Model area (dropdown + button) ----------------------------
        loadmodel_panel = PanedWindow(center_panel, height=20)
        loadmodel_panel["bg"] = "yellow"
        loadmodel_panel.pack(fill=BOTH, side=TOP)

        # Bọc bằng Frame để dùng grid (tránh grid trực tiếp lên PanedWindow)
        loadmodel_frame = Frame(loadmodel_panel, bg="yellow")
        loadmodel_panel.add(loadmodel_frame)

        # Nạp danh sách model từ thư mục /model
        model_files = self.load_model_files()  # ví dụ: ['----Select a trained machine learning model----', 'housingmodel_2025-11-05_07-30-10.zip', ...]
        self.selected_model = StringVar(self.root)
        self.selected_model.set(model_files[0])

        # Dropdown
        self.model_menu = OptionMenu(loadmodel_frame, self.selected_model, *model_files)
        self.model_menu.grid(row=0, column=0, padx=6, pady=4)

        # Nút Load
        loadmodel_button = Button(loadmodel_frame, text="6. Load Model", command=self.do_load_model)
        loadmodel_button.grid(row=0, column=1, padx=6, pady=4)
    ...
    def do_save_model(self):
        ...
        # LƯU Ý: FileUtil phải có hàm savemodel(model, path)
        FileUtil.savemodel(self.lm, path)

        # Thêm ngay vào OptionMenu hiện tại
        menu = self.model_menu["menu"]
        menu.add_command(label=filename, command=lambda md=filename: self.selected_model.set(md))

        messagebox.showinfo("info", "Exported model to disk successful!")

    def do_load_model(self):
        import os
        md = self.selected_model.get()
        if not md.endswith(".zip"):
            messagebox.showwarning("Warning", "You have to select a trained machine learning model")
            return
        path = os.path.join("model", md)
        self.lm = FileUtil.loadmodel(path)
        messagebox.showinfo("info", "Loading model from disk successful!")

    def load_model_files(self):
        hint = "----Select a trained machine learning model----"
        import os
        os.makedirs("model", exist_ok=True)
        model_files = [
            f for f in os.listdir("model")
            if os.path.isfile(os.path.join("model", f)) and f.endswith(".zip")
        ]
        model_files.sort()
        model_files.insert(0, hint)
        return model_files
